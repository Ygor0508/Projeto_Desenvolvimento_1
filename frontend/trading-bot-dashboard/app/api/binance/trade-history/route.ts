...
// import { NextResponse } from "next/server"
// import Binance from "binance-api-node" // Correção: Importação padrão
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/binance/trade-history`, {
      method: 'GET',
    ...
    if (!response.ok) {
        return NextResponse.json({ success: false, trades: [], stats: {} }, { status: response.status });
    }
    ...
    console.error("Erro no Proxy de Trade History:", error);
    return NextResponse.json({ success: false, trades: [], stats: {} }, { status: 500 });
  }
}