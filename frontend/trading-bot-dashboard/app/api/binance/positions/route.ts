...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/binance/positions`, {
      method: 'GET',
    ...
    if (!response.ok) {
        return NextResponse.json({ positions: [] }, { status: response.status });
    }
    ...
    console.error("Erro no Proxy de Positions:", error);
