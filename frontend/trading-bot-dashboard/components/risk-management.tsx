...
// ULTIMO FUNCIONANDO MAS AINDA COM ALGUNS BUGS
...
export function RiskManagement() {
  ...
  useEffect(() => {
    // Busca Saldo
    fetch("/api/balance").then(res => res.json()).then(data => setBalance(data.balance)).catch(console.error)
    ...
    fetch("/api/bot/status").then(res => res.json()).then(data => {
        if(data.positions) {
            const pos = Object.values(data.positions)
            setOpenPositionsCount(pos.length)
            setTotalExposure(pos.length * 10)
        }
    }).catch(console.error)
  }, [])
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({ 
            title: "Sucesso!", 
            description: "Configurações salvas no banco de dados.", 
            className: "bg-green-600 text-white border-none" 
        })
      } else {
        toast({ title: "Erro", description: "Falha ao salvar.", variant: "destructive" })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="bg-slate-800 border-slate-700 border-l-4 border-l-green-500">
            <CardHeader>
                ...
                <CardDescription className="text-slate-400">
                    Quanto comprar por operação (USDT fixo)
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="relative">
      ...
      <Card className="bg-slate-800 border-slate-700 border-l-4 border-l-blue-500">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-white">
            <Settings2 className="w-5 h-5" /> Filtro de Tendência Macro
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-700">
            <div className="space-y-1">
              <Label className="text-white text-base">Ativar Filtro Macro (EMA 50 Diária)</Label>
              <p className="text-sm text-slate-400">Só autoriza compras se o mercado estiver em tendência de alta.</p>
            </div>
            <Switch 
                checked={useMacroRegime} 
                onCheckedChange={setUseMacroRegime} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-slate-800 border-slate-700">
          ...
          <CardContent className="space-y-6">
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <Label className="text-slate-200">Stop Loss (%)</Label>
                    <Switch checked={stopLossEnabled} onCheckedChange={setStopLossEnabled} />
                </div>
                {stopLossEnabled && (
                <div className="space-y-2 p-3 bg-slate-900/30 rounded-md border border-slate-700/50">
                    ...
                    <Slider value={stopLossPercent} onValueChange={setStopLossPercent} max={10} min={0.5} step={0.1} className="w-full"/>
                </div>
                )}
            </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <Label className="text-slate-200">Take Profit (%)</Label>
                    <Switch checked={takeProfitEnabled} onCheckedChange={setTakeProfitEnabled} />
                </div>
                {takeProfitEnabled && (
                <div className="space-y-2 p-3 bg-slate-900/30 rounded-md border border-slate-700/50">
                    ...
                    <Slider value={takeProfitPercent} onValueChange={setTakeProfitPercent} max={20} min={1} step={0.5} className="w-full"/>
                </div>
                )}
            </div>
          </CardContent>
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-white">
                <TrendingUp className="w-5 h-5" /> Limites Operacionais
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2 pt-2">
              <Label className="text-slate-200">Máximo de Posições Abertas (Simultâneas)</Label>
              <Input 
                type="number" 
                value={maxOpenPositions} 
                onChange={(e) => setMaxOpenPositions(Number(e.target.value))} 
